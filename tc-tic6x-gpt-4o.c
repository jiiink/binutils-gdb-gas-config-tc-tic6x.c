...
static bool is_valid_pid_arg(const char *arg, unsigned int *index) {
...
static void tic6x_use_pid(const char *arg) {
  unsigned int index;
  if (is_valid_pid_arg(arg, &index)) {
    tic6x_pid = tic6x_pid_types[index].attr;
  } else {
...
int md_parse_option(int c, const char *arg) {
    switch (c) {
        ...
        case OPTION_MGENERATE_REL:
            if (c == OPTION_MDSBT) {
                tic6x_dsbt = 1;
            } else if (c == OPTION_MPID) {
                tic6x_use_pid(arg);
            } else if (c == OPTION_MPIC) {
                tic6x_pic = 1;
            } else {
...
void md_show_usage(FILE *stream) {
    ...
    fputc('\n', stream);
    fprintf(stream, "TMS320C6000 options:\n");
    fprintf(stream, "  -march=ARCH             enable instructions from architecture ARCH\n");
    fprintf(stream, "  -mbig-endian            generate big-endian code\n");
    fprintf(stream, "  -mlittle-endian         generate little-endian code\n");
    fprintf(stream, "  -mdsbt                  code uses DSBT addressing\n");
    fprintf(stream, "  -mno-dsbt               code does not use DSBT addressing\n");
    fprintf(stream, "  -mpid=no                code uses position-dependent data addressing\n");
    fprintf(stream, "  -mpid=near              code uses position-independent data addressing,\n"
                    "                            GOT accesses use near DP addressing\n");
    fprintf(stream, "  -mpid=far               code uses position-independent data addressing,\n"
                    "                            GOT accesses use far DP addressing\n");
    fprintf(stream, "  -mpic                   code addressing is position-independent\n");
    fprintf(stream, "  -mno-pic                code addressing is position-dependent\n");

    fputc('\n', stream);
    fprintf(stream, "Supported ARCH values are:");
    unsigned int arch_count = ARRAY_SIZE(tic6x_arches);
    for (unsigned int i = 0; i < arch_count; i++) {
...
static void tic6x_update_features(void) {
    ...
    bool is_arch_c64x = (tic6x_arch_enable & TIC6X_INSN_C64X) != 0;
    bool is_arch_c67xp = (tic6x_arch_enable & TIC6X_INSN_C67XP) != 0;
    bool is_arch_c64xp = (tic6x_arch_enable & TIC6X_INSN_C64XP) != 0;

    tic6x_num_registers = (is_arch_c64x || is_arch_c67xp) ? 32 : 16;
    tic6x_predicate_a0 = is_arch_c64x;
    tic6x_can_cross_fp_boundary = is_arch_c64x || is_arch_c67xp;
    tic6x_long_data_constraints = !is_arch_c64x;
    tic6x_compact_insns = is_arch_c64xp;
}
...
static void s_tic6x_cantunwind(int ignored ATTRIBUTE_UNUSED) {
    ...
    if (unwind->data_bytes == 0 || unwind->data_bytes == -1) {
...
static void s_tic6x_handlerdata(int ignored ATTRIBUTE_UNUSED) {
    ...
    if (unwind->table_entry || unwind->personality_index < -1) {
        as_bad(_("duplicate .handlerdata directive"));
        return;
    }

    if (unwind->personality_index == -1 && !unwind->personality_routine) {
...
static void s_tic6x_endp(int /*ignored*/) {
    tic6x_unwind_info *unwind = tic6x_get_unwind();
    ...
    if (unwind->data_bytes > 0 && !unwind->table_entry) {
...
static void s_tic6x_personality(int ignored ATTRIBUTE_UNUSED) {
    ...
    if (!unwind->personality_routine) {
        as_bad(_("Failed to find or make symbol"));
    ...
    if (!demand_empty_rest_of_line()) {
        as_bad(_("Unexpected tokens after .personality directive"));
        return;
    }
}
...
static void s_tic6x_arch(int ignored ATTRIBUTE_UNUSED) {
    ...
    while (*input_line_pointer && !is_end_of_stmt(*input_line_pointer) && !is_whitespace(*input_line_pointer)) {
        input_line_pointer++;
    }
...
static void s_tic6x_ehtype(int ignored ATTRIBUTE_UNUSED) {
    ...
    p = frag_more(4);
    if (p == NULL) {
        as_bad(_("failed to allocate memory"));
        return;
    }
    memset(p, 0, 4);
    fix_new_exp(frag_now, p - frag_now->fr_literal, 4, &exp, 0, BFD_RELOC_C6000_EHTYPE);
...
static void s_tic6x_nocmp(int ignored)
{
  (void)ignored; // Explicitly ignore parameter
  segment_info_type *seg_ptr = seg_info(now_seg); // Use a temporary pointer for clarity
  if (seg_ptr != NULL) { // Check for null pointer
    seg_ptr->tc_segment_info_data.nocmp = true;
  }
  demand_empty_rest_of_line();
}
...
static void s_tic6x_scomm(int ignore ATTRIBUTE_UNUSED) {
    ...
    *p = c;
    ...
    if (symbol_get_obj(symbolP)->local) {
        ...
        if (align2) frag_align(align2, 0, 0);
...
int tic6x_convert_symbolic_attribute(const char *name) {
    ...
    size_t attributes_count = ARRAY_SIZE(tic6x_attributes);
...
// Utility functions and structures assumed to be declared elsewhere
void *XNEW(size_t size);
void *str_htab_create(void);
void *str_hash_find(void *hash, const char *str);
void str_hash_insert(void *hash, const char *str, void *item, bool replace);
flagword bfd_applicable_section_flags(void *output);
void bfd_set_section_flags(void *section, flagword flags);
void bfd_set_arch_mach(void *output, int arch, int mach);
void subseg_set(segT seg, subsegT subseg);
void seg_info(void *section);
void subseg_new(const char *name, int subseg);

typedef int tic6x_opcode_id;
#define tic6x_opcode_max 100 // Placeholder
typedef void *segT;          // Placeholder types
typedef void *subsegT;       // Placeholder types
typedef int flagword;
#define SEC_ALLOC 0          // Placeholder
typedef struct {
    const char *name;
    struct {
      const char *name;
...
void *now_seg;
subsegT now_subseg;
section_t *sbss_section, scom_section;
void *opcode_hash, *stdoutput;
section_t *bfd_com_section_ptr;
struct {
    const char *name;
} scom_symbol, tic6x_opcode_table[tic6x_opcode_max];
...
void md_begin(void) {
    ...
    for (id = 0; id < tic6x_opcode_max; id++) {
        tic6x_opcode_list *opc = (tic6x_opcode_list *)XNEW(sizeof(tic6x_opcode_list));
        if (opc == NULL) {
            continue; // Handle allocation failure gracefully
        }
        opc->id = id;
        opc->next = str_hash_find(opcode_hash, tic6x_opcode_table[id].name);
        str_hash_insert(opcode_hash, tic6x_opcode_table[id].name, opc, true);
    }
...
int tic6x_unrecognized_line(int c) {
    switch (c) {
        ...
        case '[': {
            char *p = input_line_pointer;
            bool areg = false, bad_predicate = false;
            unsigned int z = (*p == '!') ? (p++, 1) : 0;

            while (*p != ']' && !is_end_of_stmt(*p)) p++;
            if (*p != ']') return 0;

            char *endp = p + 1;
            p = input_line_pointer + z;
            if (*p == 'A' || *p == 'a') {
                areg = true;
            } else if (*p == 'B' || *p == 'b') {
                areg = false;
...
void tic6x_frob_label(symbolS *sym) {
    ...
    if (!si) {
        as_internal_error(_("Failed to retrieve segment info."));
        return;
    }

    tic6x_label_list *new_label = XNEW(tic6x_label_list);
    if (!new_label) {
        as_internal_error(_("Memory allocation failed for label list."));
        return;
    }
...
static void tic6x_end_of_line(void) {
    if (tic6x_line_parallel) {
        as_bad("'||' not followed by instruction");
        tic6x_line_parallel = false;
        tic6x_line_spmask = false;
    } else if (tic6x_line_creg) {
        as_bad("predicate not followed by instruction");
        tic6x_line_creg = 0;
        tic6x_line_z = 0;
    }
}
...
void tic6x_cleanup(void) {
    if(tic6x_end_of_line() != 0) {
        // Handle potential error when cleaning up
    }
}
...
void tic6x_init_after_args(void) {
    elf32_tic6x_set_use_rela_p(stdoutput, tic6x_generate_rela != 0);
}
...
static void tic6x_free_label_list(tic6x_label_list *list) {
    while (list != NULL) {
        tic6x_label_list *next = list->next;
        free(list);
        list = next;
    }
}
...
bool tic6x_do_align(int n, char *fill, int len ATTRIBUTE_UNUSED, int max) {
    if (fill != NULL || n <= 0 || n > 5 || max < 0 || max >= (1 << n) || need_pass_2 || !subseg_text_p(now_seg)) {
        return false;
    }
...
bool tic6x_parse_register(char **p, tic6x_register *reg) {
    if (!p || !*p || !reg) return false;
    char *r = *p;
...
static bool tic6x_parse_func_unit_base(char *p, tic6x_func_unit_base *base, unsigned int *side) {
    if (!p || !base || !side) return false;

    char unit = tolower((unsigned char)p[0]);
    char num = p[1];
    ...
    switch (unit) {
        case 'd':
            *base = tic6x_func_unit_d;
            break;
        case 'l':
            *base = tic6x_func_unit_l;
            break;
        case 'm':
            *base = tic6x_func_unit_m;
            break;
        case 's':
            *base = tic6x_func_unit_s;
            break;
        default:
            return false;
    }

    *side = (unsigned int)(num - '0');
    return true;
}
...
static bool tic6x_parse_operand(char **p, tic6x_operand *op, unsigned int op_forms, char *str, int opc_len, unsigned int opno) {
    char *q = *p;
    bool operand_parsed = false;
    
    if ((op_forms & (TIC6X_OP_MEM_NOUNREG | TIC6X_OP_MEM_UNREG)) == (TIC6X_OP_MEM_NOUNREG | TIC6X_OP_MEM_UNREG))
        abort();

    if ((op_forms & TIC6X_OP_FUNC_UNIT) && !operand_parsed) {
        tic6x_func_unit_base base;
        unsigned int side;
    ...
    if ((op_forms & TIC6X_OP_IRP) && !operand_parsed) {
        if (!strncasecmp("irp", q, 3)) {
            char *rq = q + 3;
            skip_whitespace(rq);
            if (is_end_of_stmt(*rq) || *rq == ',') {
    ...
    if ((op_forms & TIC6X_OP_NRP) && !operand_parsed) {
        if (!strncasecmp("nrp", q, 3)) {
            char *rq = q + 3;
            skip_whitespace(rq);
            if (is_end_of_stmt(*rq) || *rq == ',') {
    ...
    if ((op_forms & (TIC6X_OP_MEM_NOUNREG | TIC6X_OP_MEM_UNREG)) && !operand_parsed) {
        char *mq = q;
        tic6x_mem_mod mem_mod = tic6x_mem_mod_none;
        tic6x_register base_reg;
        bool require_offset = false, permit_offset = false, offset_is_reg = false;
        ...
        if (mem_ok) {
            ...
            skip_whitespace(mq);
            mem_mod = 
                (*mq == '+' && mq[1] == '+') ? tic6x_mem_mod_preinc : 
                (*mq == '-' && mq[1] == '-') ? tic6x_mem_mod_predec : 
                (*mq == '+') ? tic6x_mem_mod_plus : 
                (*mq == '-') ? tic6x_mem_mod_minus : 
                tic6x_mem_mod_none;

            if (mem_mod != tic6x_mem_mod_none) mq += (mem_mod == tic6x_mem_mod_plus || mem_mod == tic6x_mem_mod_minus) ? 1 : 2;
        }

        if (mem_ok = tic6x_parse_register(&mq, &base_reg)) {
            if (mem_mod == tic6x_mem_mod_none) {
                skip_whitespace(mq);
                mem_mod = 
                    (mq[0] == '+' && mq[1] == '+') ? tic6x_mem_mod_postinc : 
                    (mq[0] == '-' && mq[1] == '-') ? tic6x_mem_mod_postdec : 
                    mem_mod;
                if (mem_mod != tic6x_mem_mod_none) mq += 2;
            }
            ...
            if (permit_offset) {
                ...
                if (scaled != tic6x_offset_none) {
                    skip_whitespace(mq);
                    if ((scaled == tic6x_offset_scaled || (op_forms & TIC6X_OP_MEM_UNREG)) && (offset_is_reg = tic6x_parse_register(&mq, &offset_reg))) {
                        if (*mq == endc) mq++;
                    } else {
                        expressionS tmp_exp;
        ...
        if (mem_ok && require_offset && scaled == tic6x_offset_none) mem_ok = false;

        if (mem_ok) {
            skip_whitespace(mq);
            if (!is_end_of_stmt(*mq) && *mq != ',') mem_ok = false;
        }
        ...
        if (mem_ok) {
            ...
            if (offset_is_reg) op->value.mem.offset.reg = offset_reg;
            else op->value.mem.offset.exp = offset_exp;
            operand_parsed = true;
            q = mq;
            if (base_reg.num >= tic6x_num_registers) as_bad(_("register number %u not supported on this architecture"), base_reg.num);
            if (offset_is_reg && offset_reg.num >= tic6x_num_registers) as_bad(_("register number %u not supported on this architecture"), offset_reg.num);
        }
    }

    if ((op_forms & (TIC6X_OP_REG | TIC6X_OP_REGPAIR)) && !operand_parsed) {
        ...
        if (reg_ok) {
            if (*rq == ':' && (op_forms & TIC6X_OP_REGPAIR)) {
                ...
                if (reg_ok && *rq == ',' && (second_reg.num & 1) == 0 &&
                    (first_reg.num == second_reg.num + 1) && (first_reg.side == second_reg.side)) {
                    operand_parsed = true;
                    op->form = TIC6X_OP_REGPAIR;
                    op->value.reg = second_reg;
                    q = rq;
                }
            } else if (op_forms & TIC6X_OP_REG && (*rq == ',' || is_end_of_stmt(*rq))) {
                operand_parsed = true;
            ...
            if (operand_parsed) {
                if (first_reg.num >= tic6x_num_registers) as_bad(_("register number %u not supported on this architecture"), first_reg.num);
                if (op->form == TIC6X_OP_REGPAIR && second_reg.num >= tic6x_num_registers) as_bad(_("register number %u not supported on this architecture"), second_reg.num);
            }
        }
    }

    if ((op_forms & TIC6X_OP_EXP) && !operand_parsed) {
...
int tic6x_parse_name(const char *name, expressionS *exprP, enum expr_mode mode ATTRIBUTE_UNUSED, char *nextchar) {
    char *p = input_line_pointer;
    ...
    *input_line_pointer = *nextchar;
    ...
    if (!is_name_beginner(*p)) return 0;
    char *name_start = p;
    p++;
    while (is_part_of_name(*p)) p++;
    char *name_end = p;
    skip_whitespace(p);
...
static void tic6x_fix_new_exp(fragS *frag, int where, int size, expressionS *exp,
    ...
    if (new_reloc == BFD_RELOC_UNUSED) {
        ...
        fix = fix_new(frag, where, size, exp->X_add_symbol, exp->X_add_number, pcrel, new_reloc);
    }
...
static unsigned int tic6x_coarse_operand_form(tic6x_operand_form form) {
    switch (form) {
        ...
        default:
            /* Ensure proper error handling instead of abort() */
            exit(EXIT_FAILURE);
    }
}
...
/* Return whether an operand matches the given fine-grained form and
   ...
   DATA_SIDE.  */
static tic6x_operand_match tic6x_operand_matches_form(const tic6x_operand *op, tic6x_operand_form form, tic6x_rw rw, unsigned int side, unsigned int cross, unsigned int data_side) {
    ...
    if (form == tic6x_operand_asm_const) {
        return (op->value.exp.X_op == O_constant) ? tic6x_match_matches : tic6x_match_non_const;
    }

    if (form == tic6x_operand_link_const || form == tic6x_operand_irp || form == tic6x_operand_nrp || form == tic6x_operand_func_unit) {
        return tic6x_match_matches;
    }

    if (form == tic6x_operand_reg || form == tic6x_operand_regpair) {
        return (op->value.reg.side == side) ? tic6x_match_matches : tic6x_match_wrong_side;
    }

    if (form == tic6x_operand_xreg || form == tic6x_operand_xregpair) {
        return (op->value.reg.side == cross) ? tic6x_match_matches : tic6x_match_wrong_side;
    }

    if (form == tic6x_operand_dreg || form == tic6x_operand_dregpair) {
        return (op->value.reg.side == data_side) ? tic6x_match_matches : tic6x_match_wrong_side;
    }

    if (form == tic6x_operand_areg) {
        if (op->value.reg.side != cross) return tic6x_match_wrong_side;
        return (op->value.reg.side == 2 && (op->value.reg.num == 14 || op->value.reg.num == 15)) ? tic6x_match_matches : tic6x_match_bad_address;
    }

    if (form == tic6x_operand_retreg) {
        if (op->value.reg.side != side) return tic6x_match_wrong_side;
        return (op->value.reg.num == 3) ? tic6x_match_matches : tic6x_match_bad_return;
    }

    if (form == tic6x_operand_ctrl) {
        switch (rw) {
            case tic6x_rw_read:
                return (tic6x_ctrl_table[op->value.ctrl].rw == tic6x_rw_read || tic6x_ctrl_table[op->value.ctrl].rw == tic6x_rw_read_write) ? tic6x_match_matches : tic6x_match_ctrl_write_only;
            case tic6x_rw_write:
                return (tic6x_ctrl_table[op->value.ctrl].rw == tic6x_rw_write || tic6x_ctrl_table[op->value.ctrl].rw == tic6x_rw_read_write) ? tic6x_match_matches : tic6x_match_ctrl_read_only;
            default:
                abort();
        }
    }

    if (form == tic6x_operand_mem_deref) {
        if (op->value.mem.mod != tic6x_mem_mod_none) return tic6x_match_bad_mem;
        if (op->value.mem.scaled != tic6x_offset_none) abort();
        return (op->value.mem.base_reg.side == side) ? tic6x_match_matches : tic6x_match_bad_mem;
    }

    if (form == tic6x_operand_mem_short || form == tic6x_operand_mem_ndw) {
        if (op->value.mem.base_reg.side != side) return tic6x_match_bad_mem;
        if (op->value.mem.mod == tic6x_mem_mod_none) {
            if (op->value.mem.scaled != tic6x_offset_none) abort();
            return tic6x_match_matches;
        }
        if (op->value.mem.scaled == tic6x_offset_none) {
            if (op->value.mem.mod == tic6x_mem_mod_plus || op->value.mem.mod == tic6x_mem_mod_minus) abort();
            return tic6x_match_matches;
        }
        if (op->value.mem.offset_is_reg) {
            if (op->value.mem.scaled == tic6x_offset_unscaled && form != tic6x_operand_mem_ndw) abort();
    ...
    if (form == tic6x_operand_mem_long) {
        if (op->value.mem.base_reg.side == 2 && (op->value.mem.base_reg.num == 14 || op->value.mem.base_reg.num == 15)) {
            if (op->value.mem.mod == tic6x_mem_mod_none) {
                if (op->value.mem.scaled != tic6x_offset_none) abort();
                return tic6x_match_matches;
            }
            if (op->value.mem.mod == tic6x_mem_mod_plus) {
                if (op->value.mem.scaled == tic6x_offset_none) abort();
                if (op->value.mem.offset_is_reg || (op->value.mem.scaled == tic6x_offset_scaled && op->value.mem.offset.exp.X_op != O_constant)) {
                    return tic6x_match_bad_mem;
                }
                return tic6x_match_matches;
            }
            if (op->value.mem.mod != tic6x_mem_mod_minus && op->value.mem.mod != tic6x_mem_mod_preinc && op->value.mem.mod != tic6x_mem_mod_predec &&
                op->value.mem.mod != tic6x_mem_mod_postinc && op->value.mem.mod != tic6x_mem_mod_postdec) {
                abort();
            }
            return tic6x_match_bad_mem;
        }
...
static unsigned int tic6x_dpr_shift(tic6x_coding_method coding) {
    if (coding == tic6x_coding_ulcst_dpr_byte) return 0;
    if (coding == tic6x_coding_ulcst_dpr_half) return 1;
    if (coding == tic6x_coding_ulcst_dpr_word) return 2;
    abort();
}
...
static bfd_reloc_code_real_type tic6x_dpr_reloc(tic6x_coding_method coding) {
    switch (coding) {
        ...
        default:
            fprintf(stderr, "Error: undefined coding method %d\n", coding);
            exit(EXIT_FAILURE);
    }
}
...
static void handleNoMod(tic6x_mem_ref *mem_ref) {
    if (mem_ref->scaled != tic6x_offset_none) abort();
...
static void handlePlusMinusMod(const tic6x_mem_ref *mem_ref) {
    if (mem_ref->scaled == tic6x_offset_none) abort();
}

static void handleIncDecMod(tic6x_mem_ref *mem_ref) {
    if (mem_ref->scaled != tic6x_offset_none) return;
...
static unsigned int tic6x_try_encode(
    ...
    for (unsigned int fld = 0; fld < opct->num_variable_fields; ++fld) {
        ...
        switch (opct->variable_fields[fld].coding_method) {
            case tic6x_coding_ucst:
                if (operands[opno].form != TIC6X_OP_EXP) abort();
                if (operands[opno].value.exp.X_op != O_constant) abort();
                ucexp = operands[opno].value.exp;
                if (ucexp.X_add_number < 0 || ucexp.X_add_number >= (1 << fldd->bitfields[0].width)) {
                    if (print_errors) as_bad("operand %u of '%.*s' out of range", opno + 1, opc_len, str);
                    *ok = false;
                    return 0;
                }
            ...
            case tic6x_coding_scst:
                ...
                if (operands[opno].value.exp.X_op != O_constant) {
                    if (fldd->bitfields[0].low_pos != 7 || fldd->bitfields[0].width != 16) abort();
                    *fix_needed = true;
            ...
            case tic6x_coding_ulcst_dpr_word: {
                    ...
                    case TIC6X_OP_EXP:
                        if (operands[opno].value.exp.X_op == O_constant) {
                            ...
                            if (ucexp.X_add_number & ((1 << bits) - 1)) {
                                if (print_errors) as_bad("offset in operand %u of '%.*s' not divisible by %u", opno + 1, opc_len, str, 1u << bits);
                                *ok = false;
                                return 0;
                            }
                ...
                value = 0;
                if (fldd->bitfields[0].low_pos != 8 || fldd->bitfields[0].width != 15) abort();
                *fix_needed = true;
                *fix_exp = expp;
                *fix_pcrel = 0;
                *fx_r_type = tic6x_dpr_reloc(opct->variable_fields[fld].coding_method);
                *fix_adda = (operands[opno].form == TIC6X_OP_EXP) ? true : false; 
                break;
            }
            ...
            case tic6x_coding_fcyc:
                ...
                if (operands[opno].value.exp.X_op != O_constant) abort();
                if (!sploop_ii) {
                    if (print_errors) as_bad("'%.*s' instruction not in a software pipelined loop", opc_len, str);
                    *ok = false;
                    return 0;
                }
                fcyc_bits = (sploop_ii <= 1) ? 0 : (sploop_ii <= 2) ? 1 : 
                            (sploop_ii <= 4) ? 2 : (sploop_ii <= 8) ? 3 : 4;
                if (fcyc_bits > fldd->bitfields[0].width) abort();
                if (opct->variable_fields[fld].coding_method == tic6x_coding_fstg) {
                    ...
                    int t = 0;
                    for (int i = fcyc_bits; i < fldd->bitfields[0].width; ++i) {
                        t = (t << 1) | (value & 1);
                        value >>= 1;
                    }
                    value = t << fcyc_bits;
    ...
    if (this_line_creg) {
        ...
        if (!creg) {
            if (print_errors) as_bad("instruction '%.*s' cannot be predicated", opc_len, str);
            *ok = false;
            return 0;
        }
...
static valueT md_chars_to_number(char *buf, int n) {
    valueT result = 0;
    if (buf == NULL || n <= 0) {
        return result; // return 0 if buffer is null or length is invalid
    }
    for (int i = 0; i < n; i++) {
        result <<= 8;
...
void md_assemble(char *str) {
  ...
  if (p == str) abort();  // Improper call if no instruction is found

  if (tic6x_arch_attribute == C6XABI_Tag_ISA_none) tic6x_arch_attribute = C6XABI_Tag_ISA_C674X;
  tic6x_seen_insns = true;

  // Reset parallel and predicate global settings
  seginfo = seg_info(now_seg);
  tic6x_line_parallel = tic6x_line_spmask = false;
  tic6x_line_creg = tic6x_line_z = 0;
  seginfo->tc_segment_info_data.label_list = NULL;

  // Search opcode
  opc_list = str_hash_find_n(opcode_hash, str, p - str);
  if (opc_list == NULL) {
    as_bad(_("unknown opcode '%.*s'"), (int)(p - str), str);
    return;
  }
  ...
  // Parse functional unit if present
  if (*p == '.') {
    ...
    if (tic6x_parse_func_unit_base(p + 1, &maybe_base, &maybe_side)) {
      if (is_whitespace(p[3]) || is_end_of_stmt(p[3]))
        p += 3;
      else if ((toupper(p[3]) == 'X') && (is_whitespace(p[4]) || is_end_of_stmt(p[4])))
        maybe_cross = 1, p += 4;
      else if (maybe_base == tic6x_func_unit_d && (toupper(p[3]) == 'T') && (p[4] == '1' || p[4] == '2') && (is_whitespace(p[5]) || is_end_of_stmt(p[5])))
        maybe_data_side = p[4] - '0', p += 5;
      else 
        goto skip_parse_func_unit;  // Skip bad function unit configuration
  ...
  skip_parse_func_unit:
    skip_whitespace(p);
  }

  // Match opcodes and determine operand forms
  ...
  for (opc = opc_list; opc; opc = opc->next) {
    if (!(tic6x_opcode_table[opc->id].isa_variants & tic6x_features) || 
      tic6x_opcode_table[opc->id].func_unit != func_unit_base || 
      (func_unit_side == 1 && (tic6x_opcode_table[opc->id].flags & TIC6X_FLAG_SIDE_B_ONLY)) || 
      (func_unit_cross && (tic6x_opcode_table[opc->id].flags & TIC6X_FLAG_NO_CROSS)) || 
      (!func_unit_data_side && (tic6x_opcode_table[opc->id].flags & (TIC6X_FLAG_LOAD | TIC6X_FLAG_STORE))) || 
      (func_unit_data_side && !(tic6x_opcode_table[opc->id].flags & (TIC6X_FLAG_LOAD | TIC6X_FLAG_STORE))) || 
      (func_unit_data_side == 1 && (tic6x_opcode_table[opc->id].flags & TIC6X_FLAG_SIDE_T2_ONLY))) continue;
    ...
    unsigned int num_operands = tic6x_opcode_table[opc->id].num_operands;
    if (tic6x_opcode_table[opc->id].flags & TIC6X_FLAG_SPMASK) {
      if (num_operands != 1 || (tic6x_opcode_table[opc->id].operand_info[0].form != tic6x_operand_func_unit)) abort();
      num_operands = 8;
      for (unsigned int i = 0; i < num_operands; i++) num_operands_permitted[i] = true, operand_forms[i] |= tic6x_coarse_operand_form(tic6x_operand_func_unit);
    } else {
      for (unsigned int i = 0; i < num_operands; i++) operand_forms[i] |= tic6x_coarse_operand_form(tic6x_opcode_table[opc->id].operand_info[i].form);
    }
    num_operands_permitted[num_operands] = true;
    if (num_operands > max_num_operands) max_num_operands = num_operands;
  }
  ...
  while (true) {
    skip_whitespace(p);
    if (is_end_of_stmt(*p)) {
      if (num_operands_read > 0) as_bad(_("missing operand after comma")), bad_operands = true;
      break;
    }
    ...
    if (!tic6x_parse_operand(&p, &operands[num_operands_read], operand_forms[num_operands_read], str, opc_len, num_operands_read + 1)) bad_operands = true;
    num_operands_read++;

    if (is_end_of_stmt(*p)) break;
    else if (*p == ',') {
      p++;
  ...
  if (!bad_operands) {
    for (unsigned int i = 0; i < num_operands_read; i++) {
      ...
      for (unsigned int j = 0; j < num_matching_opcodes; j++) {
        if (tic6x_opcode_table[opcm[j]].flags & TIC6X_FLAG_SPMASK || tic6x_opcode_table[opcm[j]].num_operands != num_operands_read) continue;
          
        unsigned int cf = tic6x_coarse_operand_form(tic6x_opcode_table[opcm[j]].operand_info[i].form);
        if (operands[i].form != cf) continue;

        coarse_ok = true;
        tic6x_operand_match this_fine_failure = tic6x_operand_matches_form(&operands[i], tic6x_opcode_table[opcm[j]].operand_info[i].form, tic6x_opcode_table[opcm[j]].operand_info[i].rw, func_unit_side, cross_side, func_unit_data_side);
        if (this_fine_failure == tic6x_match_matches) {
          fine_ok = true;
          break;
        }
        if (fine_failure == tic6x_match_matches || fine_failure > this_fine_failure) fine_failure = this_fine_failure;
      }

      if (!coarse_ok) abort();

      if (!fine_ok) {
        const char *operand_err_msgs[] = {
          "operand %u of '%.*s' not constant", 
          "operand %u of '%.*s' on wrong side", 
          "operand %u of '%.*s' not a valid return address register", 
          "operand %u of '%.*s' is write-only", 
          "operand %u of '%.*s' is read-only", 
          "operand %u of '%.*s' not a valid memory reference", 
          "operand %u of '%.*s' not a valid base address register"
        };
        as_bad(_(operand_err_msgs[fine_failure]), i + 1, opc_len, str);
        bad_operands = true;
        break;
      }
    }
  ...
  if (!bad_operands) {
    ...
    int min_rank = TIC6X_NUM_PREFER - 1, max_rank = 0;
    ...
    for (i = 0; i < num_matching_opcodes; i++) {
      ...
      if (this_matches) {
        ...
        if (opc_rank[rank] == -1u) opc_rank[rank] = i;
        else abort();  // Ensure no duplicate matches found
        found_match = true;
      }
    }
  ...
  for (int try_rank = max_rank; try_rank >= min_rank; try_rank--) {
    ...
    opcode_value = tic6x_try_encode(opcm[opc_rank[try_rank]], operands, num_operands_read, tick6_line_creg, tic6_line_z, func_unit_side, func_unit_cross, func_unit_data_side, seginfo->tc_segment_info_data.sploop_ii, &fix_exp, &fix_pcrel, &fx_r_type, &fix_adda, &fix_needed, &encoded_ok, try_rank == min_rank, str, opc_len);
    if (encoded_ok) {
  ...
  if (this_line_parallel) {
    if (insn_frag->fr_fix >= 32 || this_insn_label_list != NULL || (opct->flags & TIC6X_FLAG_FIRST) || *seginfo->tc_segment_info_data.last_insn_lsb & 0x1) {
      as_bad(_("[%.*s] Instruction Error in parallel code"), opc_len, str);
      return;
    }
    *seginfo->tc_segment_info_data.last_insn_lsb |= 0x1;
    ...
    tic6x_label_list *l;
    if (frag_now_fix() != 0) frag_wane(frag_now), frag_new(0);
  ...
  if (func_unit_base != tic6x_func_unit_nfu && (seginfo->tc_segment_info_data.func_units_used & tic6x_encode_spmask(func_unit_base, func_unit_side))) {
    as_bad(_("functional unit already used in this execute packet"));
  } else if (opct->flags & TIC6X_FLAG_SPLOOP && seginfo->tc_segment_info_data.sploop_ii) {
    as_bad(_("nested software pipelined loop"));
  } else if ((opct->flags & TIC6X_FLAG_SPKERNEL) && !seginfo->tc_segment_info_data.sploop_ii) {
    as_bad(_("'%.*s' instruction not in a software pipelined loop"), opc_len, str);
  } else if (this_line_spmask) {
    if (seginfo->tc_segment_info_data.spmask_addr == NULL) {
      ...
      unsigned int mask_bit = tic6x_encode_spmask(func_unit_base, func_unit_side) << 18;
  ...
  if (opct->flags & TIC6X_FLAG_SPMASK) seginfo->tc_segment_info_data.spmask_addr = output;
}
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED) {
    ...
    *valP = value;
    ...
    if (!fixP->fx_addsy && !fixP->fx_pcrel) fixP->fx_done = 1;
    ...
    switch (fixP->fx_r_type) {
        ...
        case BFD_RELOC_32:
            if (fixP->fx_done || !seg->use_rela_p) md_number_to_chars(buf, value, 4);
            break;

        case BFD_RELOC_16:
            if (fixP->fx_done || !seg->use_rela_p) {
                if (UINT_LESS_THAN(value + 0x8000, 0xf0000)) {
        ...
        case BFD_RELOC_8:
            if (fixP->fx_done || !seg->use_rela_p) {
                if (UINT_LESS_THAN(value + 0x80, 0x800)) {
                    *buf = value;
                } else {
        ...
        case BFD_RELOC_C6000_SBR_GOT_L16_W:
            if (fixP->fx_done || !seg->use_rela_p) {
                ...
                if (fixP->fx_r_type == BFD_RELOC_C6000_SBR_L16_H || fixP->fx_r_type == BFD_RELOC_C6000_SBR_L16_W || fixP->fx_r_type == BFD_RELOC_C6000_SBR_GOT_L16_W) {
                    shift = fixP->fx_r_type == BFD_RELOC_C6000_SBR_L16_H ? 1 : 2;
                }

                MODIFY_VALUE(newval, value, shift, 7, 16);
                if (UINT_GREATER_THAN(value + 0x8000, 0xf0000) && (fixP->fx_r_type == BFD_RELOC_C6000_ABS_S16 || fixP->fx_r_type == BFD_RELOC_C6000_SBR_S16)) {
                    as_bad_where(fixP->fx_file, fixP->fx_line, _("immediate offset out of range"));
                }

                md_number_to_chars(buf, newval, 4);
            }
            if (fixP->fx_done && fixP->fx_r_type != BFD_RELOC_C6000_ABS_S16 && fixP->fx_r_type != BFD_RELOC_C6000_ABS_L16) abort();
            break;
        ...
        case BFD_RELOC_C6000_SBR_GOT_H16_W:
            if (fixP->fx_done || !seg->use_rela_p) {
                valueT newval = md_chars_to_number(buf, 4);
                int shift = fixP->fx_r_type == BFD_RELOC_C6000_SBR_H16_H ? 17 : fixP->fx_r_type == BFD_RELOC_C6000_SBR_H16_W || fixP->fx_r_type == BFD_RELOC_C6000_SBR_GOT_H16_W ? 18 : 16;
                MODIFY_VALUE(newval, value, shift, 7, 16);
                md_number_to_chars(buf, newval, 4);
            }
            if (fixP->fx_done && fixP->fx_r_type != BFD_RELOC_C6000_ABS_H16) abort();
            break;
        ...
        case BFD_RELOC_C6000_SBR_GOT_U15_W:
            if (fixP->fx_done || !seg->use_rela_p) {
                valueT newval = md_chars_to_number(buf, 4);
                if (fixP->tc_fix_data.fix_adda && fixP->fx_done) value <<= (fixP->fx_r_type == BFD_RELOC_C6000_SBR_U15_H) ? 1 : 2;

                int shift = fixP->fx_r_type == BFD_RELOC_C6000_SBR_U15_W || fixP->fx_r_type == BFD_RELOC_C6000_SBR_GOT_U15_W ? 2 : 1;
                int mask = fixP->fx_r_type == BFD_RELOC_C6000_SBR_U15_W || fixP->fx_r_type == BFD_RELOC_C6000_SBR_GOT_U15_W ? 3 : 1;
                int limit = fixP->fx_r_type == BFD_RELOC_C6000_SBR_U15_W || fixP->fx_r_type == BFD_RELOC_C6000_SBR_GOT_U15_W ? 0x1fffc : 0xfffe;

                MODIFY_VALUE(newval, value, shift, 8, 15);
                if (value & mask) {
                    as_bad_where(fixP->fx_file, fixP->fx_line, _("immediate offset not %d-byte-aligned"), mask + 1);
                }
                if (value > limit) {
                    as_bad_where(fixP->fx_file, fixP->fx_line, _("immediate offset out of range"));
                }
            ...
            if (fixP->fx_done && fixP->fx_r_type != BFD_RELOC_C6000_SBR_U15_W) abort();
            break;

        case BFD_RELOC_C6000_DSBT_INDEX:
            if (value != 0) as_bad_where(fixP->fx_file, fixP->fx_line, _("addend used with $DSBT_INDEX"));
            if (fixP->fx_done) abort();
            break;
        ...
        case BFD_RELOC_C6000_PCR_S7:
            if (fixP->fx_done || !seg->use_rela_p) {
                valueT newval = md_chars_to_number(buf, 4);
                int shift = fixP->fx_r_type == BFD_RELOC_C6000_PCR_S21 ? 2 : 2;
                int range_offset = fixP->fx_r_type == BFD_RELOC_C6000_PCR_S21 ? 0x400000 : fixP->fx_r_type == BFD_RELOC_C6000_PCR_S12 ? 0x2000 : fixP->fx_r_type == BFD_RELOC_C6000_PCR_S10 ? 0x800 : 0x100;
                int range_limit = fixP->fx_r_type == BFD_RELOC_C6000_PCR_S21 ? 0x3ffffc : fixP->fx_r_type == BFD_RELOC_C6000_PCR_S12 ? 0x1ffc : fixP->fx_r_type == BFD_RELOC_C6000_PCR_S10 ? 0x7fc : 0xfc;

                MODIFY_VALUE(newval, value, shift, fixP->fx_r_type == BFD_RELOC_C6000_PCR_S12 ? 16 : 13, fixP->fx_r_type == BFD_RELOC_C6000_PCR_S7 ? 7 : fixP->fx_r_type % 4 + 10);

                if (value & 3) {
                    as_bad_where(fixP->fx_file, fixP->fx_line, _("PC-relative offset not 4-byte-aligned"));
                }
                if (UINT_GREATER_THAN(value + range_offset, range_limit + range_offset)) {
...
static inline void update_position(unsigned int *pos, unsigned int increment) {
    *pos = (*pos + increment) & 31;
}

static inline void update_frag_pointers(frchainS **frchp, fragS **fragp) {
    if ((*fragp)->fr_next) {
        *fragp = (*fragp)->fr_next;
    } else {
...
static inline bool check_alignment_requirement(fragS *fragp, unsigned int pos, unsigned int *want_insert) {
    unsigned int would_insert_bytes;
    if ((pos & ((1 << fragp->fr_offset) - 1)) == 0) {
        return false;
    }
    *want_insert = ((1 << fragp->fr_offset) - (pos & ((1 << fragp->fr_offset) - 1))) >> 2;
    if (*want_insert > 7 || (*want_insert & (*want_insert - 1))) {
        return false;
    }
    return true;
}

static void tic6x_adjust_section(bfd *abfd, segT section, void *dummy) {
    segment_info_type *info = seg_info(section);
    if (!info) return;

    bool have_code = false, have_non_code = false;
    frchainS *frchp;
    fragS *fragp;
        ...
        for (fragp = frchp->frch_root; fragp; fragp = fragp->fr_next) {
            switch (fragp->fr_type) {
                case rs_machine_dependent:
                    if (fragp->tc_frag_data.is_insns) have_code = true;
                    break;
                case rs_dummy:
                case rs_fill:
                    if (fragp->fr_fix > 0) have_non_code = true;
                    break;
                default:
                    have_non_code = true;
                    break;
    ...
    if (have_code && !have_non_code) {
        unsigned int want_insert = 0, want_insert_done_so_far = 0;
        unsigned int pos = 0;
        frchainS *frchp_reg[3] = {info->frchainP, info->frchainP, info->frchainP};
        fragS *fragp_reg[3] = {info->frchainP->frch_root, info->frchainP->frch_root, info->frchainP->frch_root};
        unsigned int pos_reg[3] = {0, 0, 0};

        for (frchp = info->frchainP; frchp; update_frag_pointers(&frchp, &fragp)) {
            bool go_back = false;
            if (fragp->fr_type != rs_machine_dependent) continue;

            if (fragp->tc_frag_data.is_insns && pos + fragp->fr_fix > 32 && !fragp->tc_frag_data.can_cross_fp_boundary) {
                if (want_insert) abort();
                if (pos & 3) abort();
                want_insert = (32 - pos) >> 2;
                if (want_insert > 7) abort();
                want_insert_done_so_far = 0;
                go_back = true;
            } else if (!fragp->tc_frag_data.is_insns) {
                if (!check_alignment_requirement(fragp, pos, &want_insert)) continue;
                want_insert_done_so_far = 0;
                go_back = true;
                ...
                if (max_poss_nops) {
                    unsigned int cur_want_nops = want_insert & -want_insert, max_want_nops = cur_want_nops - want_insert_done_so_far;
                    ...
                    for (unsigned int i = 0; i < do_nops; i++) {
                        ...
                        if (target_big_endian) fragp->fr_literal[fragp->fr_fix - 1] |= 0x1;
                        else fragp->fr_literal[fragp->fr_fix - 4] |= 0x1;
                        fragp->fr_fix += 4;
                        fragp->fr_var -= 4;
                    }
            ...
            update_position(&pos, fragp->fr_fix);
            update_frag_pointers(&frchp, &fragp);
            if (!(pos & 7)) { frchp_reg[0] = frchp; fragp_reg[0] = fragp; pos_reg[0] = pos; }
            if (!(pos & 15)) { frchp_reg[1] = frchp; fragp_reg[1] = fragp; pos_reg[1] = pos; }
            if (!(pos & 31)) { frchp_reg[2] = frchp; fragp_reg[2] = fragp; pos_reg[2] = pos; }
        }
    }
...
static void tic6x_set_attributes(void) {
    ...
    if (tic6x_set_attribute_int(Tag_ISA, tic6x_arch_attribute) != 0) {
        fprintf(stderr, "Error setting Tag_ISA attribute\n");
        return;
    }
    if (tic6x_set_attribute_int(Tag_ABI_DSBT, tic6x_dsbt) != 0) {
        fprintf(stderr, "Error setting Tag_ABI_DSBT attribute\n");
        return;
    }
    if (tic6x_set_attribute_int(Tag_ABI_PID, tic6x_pid) != 0) {
        fprintf(stderr, "Error setting Tag_ABI_PID attribute\n");
        return;
    }
    if (tic6x_set_attribute_int(Tag_ABI_PIC, tic6x_pic) != 0) {
        fprintf(stderr, "Error setting Tag_ABI_PIC attribute\n");
        return;
    }
}
...
void md_convert_frag(bfd *abfd, segT asec, fragS *fragp) {
  if (abfd == NULL || fragp == NULL) {
    // Handle error: invalid arguments
    return;
  }

  // Implementation logic goes here
  exit(EXIT_FAILURE); // Terminates the program with failure status
}
...
int md_estimate_size_before_relax(fragS *fragp, segT seg) {
    if (fragp == NULL || seg == UNDEFINED_SEG) {
        return -1; // Error code for incorrect parameters
    }
    // Proper implementation for estimating size can be added here.
...
arelent *tc_gen_reloc(asection *section ATTRIBUTE_UNUSED, fixS *fixp) {
    ...
    if (fixp == NULL || fixp->fx_frag == NULL || fixp->fx_addsy == NULL) {
        return NULL;
    }
...
int tic6x_regname_to_dw2regnum(char *regname) {
    ...
    int offset = -1;
    if (reg.num < 16) {
        offset = (reg.side == 1) ? 0 : 16;
    } else if (reg.num < 32) {
        offset = (reg.side == 1) ? 37 : 53;
    }

    return (offset == -1) ? -1 : reg.num + offset - ((reg.num >= 16) ? 16 : 0);
}
...
{
    const int stackPointerRegister = 31;
    const int offsetValue = 0;
    cfi_add_CFA_def_cfa(stackPointerRegister, offsetValue);
}
...
static void tic6x_start_unwind_section(const segT text_seg, int idx) {
    tic6x_unwind_info *unwind = tic6x_get_unwind();
    const char *prefix, *prefix_once, *text_name = segment_name(text_seg);
    struct elf_section_match match = {0};
    char *sec_name;
    size_t prefix_len, text_len;
    int type, flags = SHF_ALLOC, linkonce = 0;

    prefix = idx ? ELF_STRING_C6000_unwind : ELF_STRING_C6000_unwind_info;
    prefix_once = idx ? ELF_STRING_C6000_unwind_once : ELF_STRING_C6000_unwind_info_once;
    type = idx ? SHT_C6000_UNWIND : SHT_PROGBITS;

    if (streq(text_name, ".text")) text_name = "";
    ...
    sec_name = XNEWVEC(char, prefix_len + text_len + 1);
...
static void tic6x_flush_unwind_word(valueT data) {
    ...
    if (unwind->table_entry == NULL) {
        ...
        unwind->frag_start = ptr = frag_more(4);
    } else {
        ptr = frag_more(4);
    }
    ...
    if (ptr != NULL) {
        ...
        // Handle error for frag_more allocation failure (if needed)
    }
}
...
static void tic6x_unwind_byte(int byte) {
    ...
    if (unwind->data_bytes == 5) {
        ...
        if ((unwind->data_bytes % 4) == 0 && unwind->data_bytes > 4) {
            tic6x_flush_unwind_word(unwind->data);
            unwind->data = 0;
        }
    }
...
void tic6x_unwind_2byte(int bytes) {
    if (bytes < 0) return;

    unsigned char highByte = (unsigned char)(bytes >> 8);
    unsigned char lowByte = (unsigned char)(bytes & 0xff);

    tic6x_unwind_byte(highByte);
    tic6x_unwind_byte(lowByte);
}

static void tic6x_unwind_uleb(offsetT offset) {
    do {
        tic6x_unwind_byte((offset & 0x7F) | ((offset > 0x7F) ? 0x80 : 0));
        offset >>= 7;
    } while (offset != 0);
}
...
static void tic6x_output_exidx_entry(void) {
    ...
    if (!unwind) {
        return; // Ensure unwind is valid for reliability
    }
    ...
    if (!fix_new(frag_now, where, 4, unwind->function_start, 0, 1, BFD_RELOC_C6000_PREL31)) {
        return; // Ensure proper fix creation
    }
    ...
    if (unwind->personality_index >= 0 && unwind->personality_index < 5
        ...
        if (!pr || !fix_new(frag_now, where, 0, pr, 0, 1, BFD_RELOC_NONE)) {
            return; // Ensure proper symbol handling and fix creation
        }
        seg_info(now_seg)->tc_segment_info_data.marked_pr_dependency |= 1 << unwind->personality_index;
    }

    if (unwind->table_entry) {
        if (!fix_new(frag_now, where + 4, 4, unwind->table_entry, 0, 1, BFD_RELOC_C6000_PREL31)) {
            return; // Ensure proper fix creation
        }
    } else {
    ...
    subseg_set(old_seg, old_subseg); // Restore the original section
}

static void tic6x_output_unwinding(bool need_extab) {
    tic6x_unwind_info *unwind = tic6x_get_unwind();
    unsigned reg_masks = unwind->safe_mask | unwind->compact_mask | unwind->reg_saved_mask;
    ...
    if (unwind->personality_index == -1 && !unwind->personality_routine) {
        if (unwind->reg_saved_mask || cfa_offset >= MAX_COMPACT_SP_OFFSET) {
            unwind->personality_index = -1;
    ...
    if (unwind->personality_index == 3 || unwind->personality_index == 4) {
        ...
        unwind->data |= (unwind->cfa_reg == 15) ? 0x7f << 17 : cfa_offset << (17 - 3);
        unwind->data |= (unwind->personality_index == 3 ? unwind->safe_mask : unwind->compact_mask) << 4;
        ...
        if (unwind->personality_routine) {
            ...
            fix_new(frag_now, frag_now_fix() - 4, 4, unwind->personality_routine, 0, 1, BFD_RELOC_C6000_PREL31);
    ...
    if (unwind->table_entry) {
        valueT tmp;
        if (unwind->data_bytes > 0x400) as_bad(_("too many unwinding instructions"));
...
void tic6x_cfi_endproc(struct fde_entry *fde) {
    ...
    for (insn = fde->data; insn; insn = insn->next) {
        switch (insn->insn) {
            ...
            case DW_CFA_same_value:
                ...
                if (reg >= 0) unwind->reg_saved[reg] = false;
                break;
            case DW_CFA_offset:
                ...
                if (insn->u.ri.reg == UNWIND_B3) unwind->return_reg = UNWIND_B3;
                break;
    ...
    if (validate_cfa_reg_and_offset(unwind->cfa_reg, cfa_offset)) return;

    reg_saved_mask = calculate_reg_saved_mask(unwind);

    if (reg_saved_mask) {
        save_offset = 0;
        if (validate_safe_frame_layout(unwind, &save_offset)) safe_mask = reg_saved_mask;
    }

    if (reg_saved_mask) {
        if (validate_compact_frame_layout(unwind, &save_offset)) compact_mask = reg_saved_mask;
    }

    if (reg_saved_mask == 0x17ff) {
        save_offset = 0;
        if (validate_pop_rts_layout(unwind, &save_offset)) unwind->pop_rts = true;
    }

    if (reg_saved_mask) {
        save_offset = 0;
        if (!process_remaining_reg_saves(unwind, &save_offset)) return;
    }
...
int validate_cfa_reg_and_offset(int cfa_reg, offsetT cfa_offset) {
    if (cfa_reg == 15 && cfa_offset != 0) {
        as_bad(_("unable to generate unwinding opcode for frame pointer offset"));
        return -1;
    }
    if (cfa_reg == 31 && (cfa_offset & 7) != 0) {
...
unsigned calculate_reg_saved_mask(tic6x_unwind_info *unwind) {
    unsigned reg_saved_mask = 0;
    for (int reg = 0; reg < TIC6X_NUM_UNWIND_REGS; reg++) {
        if (unwind->reg_saved[reg]) reg_saved_mask |= 1 << (TIC6X_NUM_UNWIND_REGS - (reg + 1));
    }
    return reg_saved_mask;
}

int validate_safe_frame_layout(tic6x_unwind_info *unwind, offsetT *save_offset) {
    for (int reg = 0; reg < TIC6X_NUM_UNWIND_REGS; reg++) {
        if (!unwind->reg_saved[reg]) continue;
        if (target_big_endian && reg < TIC6X_NUM_UNWIND_REGS - 1 && unwind->reg_saved[reg + 1] &&
            tic6x_unwind_frame_regs[reg] == tic6x_unwind_frame_regs[reg + 1] + 1 &&
            (tic6x_unwind_frame_regs[reg] & 1) == 1 && (*save_offset & 4) == 4) {
            if (*save_offset != unwind->reg_offset[reg + 1] || *save_offset - 4 != unwind->reg_offset[reg])
                return 0;
            *save_offset -= 8;
            reg++;
        } else {
            if (*save_offset != unwind->reg_offset[reg]) return 0;
            *save_offset -= 4;
        }
    }
    return 1;
...
int validate_compact_frame_layout(tic6x_unwind_info *unwind, offsetT *save_offset) {
    int reg2;
    int high_offset;
    for (int reg = 0; reg < TIC6X_NUM_UNWIND_REGS; reg++) {
        if (!unwind->reg_saved[reg]) continue;
        reg2 = (reg < TIC6X_NUM_UNWIND_REGS - 1) && unwind->reg_saved[reg + 1] &&
               tic6x_unwind_frame_regs[reg] == tic6x_unwind_frame_regs[reg + 1] + 1 &&
               (tic6x_unwind_frame_regs[reg + 1] & 1) == 0 ? reg + 1 : -1;
        if (reg2 >= 0) {
            high_offset = target_big_endian ? 4 : 0;
            if (*save_offset + 4 - high_offset != unwind->reg_offset[reg] ||
                *save_offset + high_offset != unwind->reg_offset[reg2]) return 0;
            reg++;
        } else {
            if (*save_offset != unwind->reg_offset[reg]) return 0;
        }
...
int validate_pop_rts_layout(tic6x_unwind_info *unwind, offsetT *save_offset) {
    const int *pop_rts_offset = target_big_endian ? tic6x_pop_rts_offset_big : tic6x_pop_rts_offset_little;
    for (int reg = 0; reg < TIC6X_NUM_UNWIND_REGS; reg++) {
        if (reg == UNWIND_B15) continue;
        if (unwind->reg_offset[reg] != pop_rts_offset[reg] * 4) return 0;
    }
    return 1;
}

int process_remaining_reg_saves(tic6x_unwind_info *unwind, offsetT *save_offset) {
    for (int reg = 0; reg < TIC6X_NUM_UNWIND_REGS; reg++) {
        if (!unwind->reg_saved[reg]) continue;
        unwind->saved_reg_count++;
